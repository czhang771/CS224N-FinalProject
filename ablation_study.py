...
C_VALUES  = [0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0]
PENALTIES = ["l1", "l2"]

def tune_C(X_tr, y_tr, X_va, y_va):
    """Select penalty + C maximizing val AUC-ROC. Returns (best_C, best_penalty, scaler, clf)."""
    best_auc, best_C, best_penalty, best_scaler, best_clf = 0.0, C_VALUES[0], "l2", None, None
    for penalty in PENALTIES:
        ...
        for C in C_VALUES:
            ...
            clf = LogisticRegression(
                ...
                class_weight="balanced", solver=solver, random_state=42,
            )
            clf.fit(X_tr_s, y_tr)
            proba = clf.predict_proba(scaler.transform(X_va))[:, 1]
            auc_val = roc_auc_score(y_va, proba)
            if auc_val > best_auc:
                best_auc, best_C, best_penalty = auc_val, C, penalty
                best_scaler, best_clf = scaler, clf
    return best_C, best_penalty, best_scaler, best_clf
...
for name, Xtr_parts, Xva_parts, Xte_parts in experiments:
    ...
    print(f"  penalty={best_penalty}  C={best_C}  threshold={threshold:.2f}  F1={metrics['F1']:.4f}  AUC-ROC={metrics['AUC-ROC']:.4f}")

    results.append({"Model": name, "penalty": best_penalty, "C": best_C, **{k: v for k, v in metrics.items() if not k.startswith("_")}})
    clfs[name]    = clf
...
df_fmt = df[["penalty", "C"] + fmt_cols].copy()
...
df[["penalty", "C"] + fmt_cols].to_csv("results_ablation.csv")
