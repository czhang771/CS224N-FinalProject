...
def _attention_features(record: dict) -> dict[str, float]:
    ...
    per_head_ctx_concentration = (ctx_attn / total_attn).astype(np.float32)  # (n_layers, n_heads)
    per_layer_ctx  = per_head_ctx_concentration.mean(axis=1)                  # (n_layers,)
    ...
    return feats, per_head_ctx_concentration
...
def extract_record(record: dict) -> tuple[dict[str, float], np.ndarray, np.ndarray, int, str]:
    """Returns (scalar_feats, hidden_state, per_head_ctx, label, pubid). Raises on bad records."""
    attn_feats, per_head_ctx = _attention_features(record)
    scalar = {
    ...
    return scalar, hidden, per_head_ctx, label, pubid
